...
- [ ] Phase 3.7: Promote daemon runner to default browser entrypoint and audit policy/docs (this is the slice where the deferred transports stop being deferrable: `CdpTargetTransport`, `AuthStateProbe`, `NetworkObservationSource` are all `NotImplemented*` today, so NO real browser test is possible before it -- budget for an integration harness a fixture suite cannot replace. ACCEPTANCE CRITERION carried from the 3.6 review: the transport MUST re-apply `isSafeNavigationTarget` to every REDIRECT destination before following it AND validate the RESOLVED address at connect time (DNS rebinding) -- the entry-URL gate cannot cover redirects, the predicate cannot judge DNS names, and nothing downstream can compensate)
